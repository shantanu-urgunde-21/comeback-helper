...
class IngestionPipeline:
    ...
    def __init__(self, ocr_provider: BaseOCRProvider | None = None):
        ...
        if ocr_provider is not None:
            ...
            if provider_type == "marker":
                from .marker_provider import MarkerOCRProvider
                self.ocr_provider = MarkerOCRProvider()
            elif provider_type == "handwriting":
                from .handwriting_provider import HandwritingOCRProvider
                self.ocr_provider = HandwritingOCRProvider()
            elif provider_type == "local":
                from .local_ocr import LightOnOCRProvider
    ...
    def process_pdf(
        ...
        # Build into a hidden sidecar and only move it into place once OCR
        # succeeds. Writing straight to target_path truncated any existing
        # note before the first OCR call, so a mid-run failure destroyed
        # good content and left a header-only stub behind.
