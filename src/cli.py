...
@main.command(name="graph-migrate-identity")
@click.option("--yes", is_flag=True, help="Skip the confirmation prompt (still writes a .bak of graph.json first)")
@_JSON
def graph_migrate_identity(yes: bool, as_json: bool):
    """Migrate the live graph onto the deterministic identity layer (plan.md Phase 2).

    Resolves every existing node (and its recorded aliases) through the same
    ladder new extractions use, collapsing spelling duplicates onto one
    canonical id, and drops known debris (retired note-container nodes,
    relation names, placeholder labels, LaTeX fragments). One-time,
    idempotent. Writes graph.json.bak before saving.
    """
    try:
        from src.wiring import build_indexer
        indexer = build_indexer()
        ...
        if not as_json and not yes:
            console.print(
                f"[yellow]About to migrate {indexer.graph.number_of_nodes()} nodes in "
                f"{indexer.graph_file}. A .bak copy will be written first.[/yellow]"
            )
            click.confirm("Proceed?", abort=True)

        backup_path = indexer.graph_file.with_suffix(".json.bak")
        if indexer.graph_file.exists():
            backup_path.write_text(indexer.graph_file.read_text(encoding="utf-8"), encoding="utf-8")

        result = indexer.migrate_to_identity_layer()
        indexer.save_graph()

        if as_json:
            _emit({"status": "success", "command": "graph-migrate-identity", "backup": str(backup_path), **result})
        else:
            console.print(Panel(
                f"[bold green]Migration complete.[/bold green]\n"
                f"Dropped junk: {result['dropped_junk_nodes']}\n"
                f"Merged away: {result['nodes_merged_away']}\n"
                f"Final: {result['final_node_count']} nodes, {result['final_edge_count']} edges\n"
